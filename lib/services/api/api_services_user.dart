...
Future<InformationMasCustomners> apiVerifyChangeDevice(
    {dynamic postData}) async {
  print("apiVerifyChangeDevice");
  InformationMasCustomners dataCustomers;
  String urlPost = "$baseUrl/User/VerifyChangeDevice"; //User/CheckVerifyPhone;
  try {
    final response = await http.post(urlPost, body: postData);
    if (response.statusCode == 200) {
      print(
...
Future<ModelVerifyNumber> apiChangeDevice({dynamic postData}) async {
...
Future<ModelVerifyLogin> apiVerifyLogin({dynamic postData}) async {
  print("apiLogin");
